  ...
  if (Auth_IsUserLoggedIn(clientIP)) {
    ...
    if (LittleFS.exists("/filemanager.html")) {
      AsyncWebServerResponse *response =
          request->beginResponse(LittleFS, "/filemanager.html", "text/html");
    ...
    if (LittleFS.exists("/login.html")) {
      AsyncWebServerResponse *response =
          request->beginResponse(LittleFS, "/login.html", "text/html");
      request->send(response);
    } else {
...
void Server_HandleGetSettings(AsyncWebServerRequest *request) {
  ...
  String defaultSettings =
      R"({"username":"admin","password":"admin","apSSID":"ESP","apPassword":"12345678","staSSID":"Jan","staPassword":"Asdf1234!"})";
...
void Server_Setup() {
  server.on("/", HTTP_GET,
            [](AsyncWebServerRequest *request) { Server_HandleRoot(request); });
  ...
  server.on(
      ...
      [](AsyncWebServerRequest *request, uint8_t *data, size_t len,
        ...
        if (index + len == total) {
          ...
          if (!error && doc.containsKey("username") &&
            ...
            if (Auth_Login(username, password, clientIP)) {
              ...
              Serial.printf(
                  "❌ Login failed - invalid credentials from IP: %s\n",
                  clientIP.c_str());
            }
          } else {
  ...
  // ============ ESP32 STATUS ENDPOINT (REAL-TIME) ============
  server.on("/api/esp32status", HTTP_GET, [](AsyncWebServerRequest *request) {
    // Check authentication (IP-only)
    if (!ValidateIPFromRequest(request)) {
      request->send(401, "application/json", "{\"error\":\"Unauthorized\"}");
      return;
    ...
    String uptimeStr;
    if (days > 0) {
      uptimeStr = String(days) + "d " + String(hours % 24) + "h";
    } else if (hours > 0) {
      uptimeStr = String(hours) + "h " + String(minutes % 60) + "m";
    } else {
      uptimeStr = String(minutes) + "m " + String(seconds % 60) + "s";
    }

    // Get free memory in KB
    size_t freeHeap = ESP.getFreeHeap();
    String memoryStr = String(freeHeap / 1024) + " KB";

    // Build JSON response
    String json = "{";
    json += "\"icon\":\"" + esp32StatusIcon + "\",";
    json += "\"message\":\"" + esp32Status + "\",";
    json += "\"uptime\":\"" + uptimeStr + "\",";
    json += "\"freeMemory\":\"" + memoryStr + "\",";
    json += "\"timestamp\":\"" + NTP_GetFormattedTimestamp() + "\"";
    json += "}";
