...
String NTP_GetFormattedTimestamp() {
  time_t now = time(nullptr);
  struct tm timeinfo;
  ...
  sprintf(buffer, "%02d %s %04d %02d:%02d %s", timeinfo.tm_mday,
          months[timeinfo.tm_mon], timeinfo.tm_year + 1900, hour12,
...
void LoadWiFiSettings(String &staSSID, String &staPassword, String &apSSID,
                      String &apPassword) {
...
void WiFi_STA_Reconnect_Handler() {
  ...
  if (WiFi.status() != WL_CONNECTED &&
      millis() - lastSTAReconnect > STA_RECONNECT_INTERVAL) {
