...
UserAuth user_auth(FIREBASE_API_KEY, FIREBASE_USER_EMAIL,
...
bool Firebase_VerifyUser(const String &apiKey, const String &email,
  ...
  if (ssl_client.connect(host.c_str(), 443) > 0) {
    ...
    String header =
        "POST /identitytoolkit/v3/relyingparty/verifyPassword?key=" + apiKey +
        " HTTP/1.1\r\n";
    header += "Host: " + host + "\r\n";
    ...
    if (ssl_client.print(header) == header.length() &&
        ssl_client.print(payload) == payload.length()) {
...
void Firebase_Init() {
  ...
  if (!Firebase_VerifyUser(FIREBASE_API_KEY, FIREBASE_USER_EMAIL,
                           FIREBASE_USER_PASSWORD)) {
    Serial.println("❌ FAILED!");
    Serial.println(
        "   Firebase authentication failed. Check credentials in MyFirebase.h");
    return;
  }
  ...
  initializeApp(aClient, app, getAuth(user_auth), Firebase_PrintResult,
                "authTask");
...
bool Firebase_IsReady() { return firebaseReady && app.ready(); }
...
bool Firebase_ResetConnection() {
  ...
  if (firebaseReady) {
    ...
    nextReconnectAttempt =
        millis() + 30000; // avoid thrashing, schedule next attempt
    return false;
  }
}
...
bool Firebase_EnsureReady(unsigned long maxWaitMs) {
  ...
  if (consecutiveFailures >= MAX_FAILURES) {
    Serial.printf("❌ Firebase failed %d times, giving up until restart\n",
                  consecutiveFailures);
    return false;
  }
  ...
  if (!Firebase_IsReady()) {
    ...
    Serial.printf("❌ Firebase still not ready (failure %d/%d)\n",
...
// bool UpdateFlight(const String &category, const String &flightKey, const
// JsonDocument &fieldsToUpdate){
//   if (!Firebase_IsReady()) {
//     Serial.println("❌ Firebase not ready. Call Firebase_Init() first.");
//     return;
//   }
//   String path = "/" + category + "/" + flightKey;
//   Database.set<object_t>(aClient, path.c_str(), object_t(jsonStr),
//   Firebase_PrintResult);
// }
bool UpdateFlight(String category, String flightKey,
  ...
  Serial.printf("  🔥 Firebase UPDATE to %s\n", path.c_str());
  Serial.printf("  📤 Payload: %s\n", jsonStr.c_str());

  // Async request
  Database.update<object_t>(aClient, path, object_t(jsonStr),
                            Firebase_PrintResult);
  ...
  if (!completed) {
    ...
    if (consecutiveTimeouts >= maxConsecutiveTimeouts) {
      ...
      if (currentTimeout < maxTimeout) {
        currentTimeout += timeoutIncrement;
        Serial.printf(
            "⏰ Increasing timeout to %d seconds for next operations\n",
            currentTimeout / 1000);
      }
      return false;
    }
...
// ============ BATCH UPDATE MULTIPLE FLIGHTS AT ONCE (FIELD-LEVEL) ============
bool UpdateFlightsBatch(String category, JsonObject flightsToUpdate) {
  ...
  // Use simple whole-object updates (ESP32 is the authoritative source)
  // Field-level updates cause memory fragmentation crashes
  String path = "/flights/Islamabad/" + category;

  // Serialize the flight objects directly
  String jsonStr;
  serializeJson(flightsToUpdate, jsonStr);

  Serial.printf("📦 Sending %d flights to %s\n", flightsToUpdate.size(),
                path.c_str());

  // Async request for batch update
  Database.update<object_t>(aClient, path.c_str(), object_t(jsonStr),
                            Firebase_PrintResult);
  ...
  if (!completed) {
    consecutiveTimeouts++;
    Serial.printf(
        "⏳ Timeout while waiting for batch update! (Timeout #%d, %ds)\n",
        consecutiveTimeouts, currentTimeout / 1000);
    ...
    if (consecutiveTimeouts >= maxConsecutiveTimeouts) {
      ...
      if (Firebase_ResetConnection()) {
        Serial.println(
            "✅ Connection reset successful, retrying batch update...");
  ...
  if (success) {
    ...
    Serial.printf("✅ Successfully updated %d %s flights!\n",
...
void Firebase_UpdateLastSyncTime() {
  if (!Firebase_IsReady()) {
    Serial.println("⚠️ Firebase not ready, skipping timestamp update");
    return;
  }
  ...
  if (timestamp == "Time not set") {
    Serial.println("⚠️ NTP time not set, skipping timestamp update");
    return;
  }

  Serial.printf("📅 Updating Esp33Update timestamp: %s\n", timestamp.c_str());

  // Reset flags
  completed = false;
  success = false;

  // Update the timestamp at ROOT level: /Esp33Update
  String path = "/Esp33Update";

  // Create a properly formatted JSON string value
  String jsonValue = "\"" + timestamp + "\"";
  object_t timestampObj(jsonValue);

  Database.set<object_t>(aClient, path.c_str(), timestampObj,
                         Firebase_PrintResult);

  // Wait for completion with 10 second timeout
  unsigned long start = millis();
  while (!completed && millis() - start < 10000) {
    app.loop();
    delay(10);
  }
