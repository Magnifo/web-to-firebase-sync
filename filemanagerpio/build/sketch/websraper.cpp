#line 1 "c:\\Users\\JanShahid\\Desktop\\filemanagerpio\\websraper.cpp"
#include "Global.h"
...
String NormalizeFlightKey(String keySTM, String flnr) {
  ...
  if (keySTM.length() != 10) {
    ...
    if (cleanSTM.length() != 10) {
      Serial.printf("⚠️ Invalid STM format: '%s' (length: %d), cannot normalize\n", 
...
time_t ParseKeySTMTime(String keySTM) {
  if (keySTM.length() != 10) return 0;
  
  struct tm timeinfo = {0};
  timeinfo.tm_year = (keySTM.substring(0, 2).toInt() + 2000) - 1900; // Year since 1900
...
void Fetch(String line, String category, String subCategory)
{
  ...
  if (phpSessionID.length() > 0)
  {
    do
    {
      ...
      delay(8500);  // ~8.5 second delay between pages (35 pages over 5 min = 300s/35 = ~8.5s per page)
    } while (Page<Pages);
...
void ExtractTDInfoJSON(String trHtml, JsonDocument &doc, String Category, String subCategory)
{
  bool Checkin = false;
  if (Category == "CheckIn")
  {
  ...
  while (true)
  {
    ...
    if (fieldName == "stm") {
      ...
      // If format is wrong, try alternative patterns
        ...
        // Handle other date formats - extract numbers only and take first 10 digits
        String cleanValue = "";
  ...
  if (key.isEmpty()) {
    Serial.printf("[SKIP] Flight key normalization failed: stm='%s', flnr='%s'\n", 
...
void postRequest(String cnfg, int pg, int pgs, String category, String subCategory)
{
  if (WiFi.status() == WL_CONNECTED)
  {
    HTTPClient http;
    ...
    if (httpCode > 0 && httpCode == HTTP_CODE_OK)
    {
      ...
      while (http.connected() && stream->available())
      {
        ...
        if (!inRow && (line.indexOf("<tr class='row_1'>") >= 0 || line.indexOf("<tr class='row_0'>") >= 0))
        {
          inRow = true;
          rowData = line;
          continue;
